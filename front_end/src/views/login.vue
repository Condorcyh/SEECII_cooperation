<template>
  <div class="main">
    <div class="top">
      <div class="header">
        <div>
          <span class="title">GreenTrip酒店管理系统</span>
        </div>
      </div>
      <div class="desc">
      ...
      >
        ...
        <a-tab-pane key="tab2" tab="注册新账号">
          ...
          <a-form-item>
            <a-input size="large"
                     type="managerCode"
                     placeholder="管理员身份校验码(非管理员身份勿填)"
                     v-decorator="[
                'managerCode',
                {rules: [{ required: false, message: '管理员请输入校验码' }], validateTrigger: 'blur'}]">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handlelogin() {
        const validateFieldsKey = this.customActiveKey === 'tab1' ? ['userName', 'password'] : ['registerUsername', 'registerUserMail','registerPassword','registerPasswordconfirm','managerCode'];
      ...
      handleRegister() {
        ...
        const validateFieldsKey = this.customActiveKey === 'tab1' ? ['userName', 'password'] : ['registerUsername','registerPhoneNumber','registerUserMail','registerPassword','registerPasswordconfirm','managerCode'];
        validateFields(validateFieldsKey, { force: true }, async (err, values) => {
          if (!err) {
            ...
            const data = {
              ...
              managerCode: this.form.getFieldValue('managerCode'),
              credit: 100,
              userType: 1
            };
            console.log(data);
